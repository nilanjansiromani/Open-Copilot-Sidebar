// Side Panel JavaScript - Firefox Sidebar Support
...
function loadUsageStats() {
  browserAPI.storage.local.get(['usageStats'], (result) => {
...
function saveUsageStats() {
  ...
  browserAPI.storage.local.set({ usageStats: statsToSave });
}
...
function loadContextToggleState() {
  browserAPI.storage.local.get(['autoSearchTabs'], (result) => {
...
function saveContextToggleState() {
  if (contextToggle) {
    ...
    browserAPI.storage.local.set({ autoSearchTabs: autoContextMode });
  }
}
...
function saveConversationHistory() {
  ...
  browserAPI.storage.local.set({ [historyKey]: conversationData }, () => {
...
function loadConversationHistory() {
  ...
  browserAPI.storage.local.get([historyKey], (result) => {
...
function clearCurrentChat() {
  ...
  if (confirm('Clear the current chat history?')) {
    browserAPI.storage.local.remove([historyKey], () => {
      messages = [];
      messagesList.innerHTML = '';
      welcomeMessageAdded = false;
      welcomeMessageAdded = true;
...
async function loadAllMentionSources(filter = '') {
  ...
  try {
    ...
    if (filterLower.length >= 1) {
      try {
        const bookmarks = await browserAPI.bookmarks.search(filter || 'a');
      ...
      // Search history
      try {
        const history = await browserAPI.history.search({ text: filter, maxResults: 10 });
...
async function selectMentionItem(index) {
  ...
  if (existingIndex >= 0) {
    ...
    if (item.type === 'bookmark' || item.type === 'history') {
      ...
      try {
        const content = await browserAPI.runtime.sendMessage({
