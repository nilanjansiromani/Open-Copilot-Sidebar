...
const AgentTools = {
  ...
  async read_page(params, tabId) {
    ...
    try {
      ...
      if (selector) {
        ...
        content = await browserAPI.scripting.executeScript({
          target: { tabId },
          func: (sel) => {
  ...
  async search_memory(params) {
    ...
    try {
      ...
      const data = await browserAPI.storage.local.get(['todos', 'notes', 'memories']);
  ...
  async focus_tab(params) {
    ...
    try {
      ...
      if (tabId) {
        targetTab = await browserAPI.tabs.get(tabId);
      } else if (url) {
        // Find tab by URL
