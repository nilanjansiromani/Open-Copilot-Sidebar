// Background service worker for Firefox Sidebar extension

console.log('OpenCopilot background service worker started');

// Use browser API if available (Firefox), fallback to chrome (Chrome/Zen)
...
browserAPI.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'saveSettings') {
    browserAPI.storage.sync.set({ settings: request.settings }, () => {
  ...
  if (request.action === 'agent_closeTab') {
    browserAPI.tabs.remove(request.tabId).then(() => {
      sendResponse({ success: true });
    }).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
...
async function extractTabContent(tabId) {
  try {
    ...
    if (results && results[0] && results[0].result) {
      ...
      const markdownResults = await browserAPI.scripting.executeScript({
        target: { tabId: tabId },
...
// Set up sidebar for Firefox
if (browserAPI.sidebarAction) {
  // Firefox sidebar is already configured in manifest.json
  // No additional setup needed
  console.log('Firefox sidebar configured');
} else if (browserAPI.sidePanel) {
  // Chrome/Zen browser fallback
